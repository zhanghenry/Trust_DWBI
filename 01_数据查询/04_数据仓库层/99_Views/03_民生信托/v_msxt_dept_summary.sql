...
select a.l_month_id,
       ...
       case when nvl(substr(b.l_setup_date,1,4),substr(f.l_setup_date,1,4)) = substr(a.l_month_id,1,4) then 'NEW' else 'EXIST' end as c_status,
       sum(case
            when e.c_dept_type = '0' and c.c_ietype_code_l1 in( 'XTSR','TZSR') then a.f_planned_eoy
 ...
 where a.l_stage_id = b.l_stage_id(+)
   ...
   and nvl(substr(b.l_setup_date,1,6),substr(f.l_setup_date,1,6)) <= a.l_month_id
   and d.l_recog_flag = 0
   and a.c_object_type = 'BM'
   /*and e.c_dept_code_l1 = '0_ms16'
   and a.l_month_id = 201712*/
   and substr(f.l_effective_date,1,6) <= a.l_month_id
   and substr(f.l_expiration_date,1,6) > a.l_month_id
   group by a.l_month_id,a.l_object_id,case when nvl(substr(b.l_setup_date,1,4),substr(f.l_setup_date,1,4)) = substr(a.l_month_id,1,4) then 'NEW' else 'EXIST' end;
