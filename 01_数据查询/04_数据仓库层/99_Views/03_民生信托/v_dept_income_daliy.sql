create or replace view v_dept_income_daliy as
with temp_main as
 (select distinct b.l_dayid as l_day_id, a.l_object_id,a.l_proj_Id, a.l_divide_id as l_stage_id
    from dim_pe_divide_scheme a, dim_day b
   where a.c_scheme_type = 'QC'
     and a.c_object_type = 'BM'
     and a.l_begdiv_date <= b.l_dayid
     and a.l_enddiv_date >= b.l_dayid
     and a.l_effective_date <= b.l_dayid
     and a.l_expiration_date > b.l_dayid
  union
  select distinct b.l_day_id, a.l_object_id, a.l_proj_Id, b.l_stage_id
   ...
   where a.l_divide_id = b.l_proj_id
     ...
     and b.l_stage_id <> 0
     and a.l_begdiv_date <= b.l_day_id
     and a.l_enddiv_date >= b.l_day_id
...
temp_revenue as
 (select t2.l_day_id, t2.l_proj_id, t2.l_stage_id, t2.l_object_id,
     ...
     SUM(CASE WHEN t21.c_ietype_code_l2 = 'XTBC' and t22.l_iestatus_id = 1 THEN t2.f_planned_eoy
              when t21.c_ietype_code in ('XTFXBC', 'TZZYFX') and t22.l_iestatus_id = 1 then t2.f_planned_eoy * -1 else 0 end) AS f_xtbc_new,
     ...
     SUM(CASE WHEN t21.c_ietype_code_l2 = 'XTCGF' and t22.l_iestatus_id = 1 THEN t2.f_planned_eoy
              when t21.c_ietype_code in ('XTFXCGF') and t22.l_iestatus_id = 1 then t2.f_planned_eoy * -1 ELSE 0 END) AS f_xtcgf_new,
     ...
     sum(case when t21.c_ietype_code_l1 = 'XTSR' and t23.l_setup_date <= substr(t2.l_day_id,1,4) * 10000 + trunc(substr(t2.l_day_id,5,2)/3) * 300 + 31
              ...
              when t21.c_ietype_code in ('XTFXBC','XTFXCGF','TZZYFX') and t23.l_setup_date <= substr(t2.l_day_id,1,4) * 10000 + trunc(substr(t2.l_day_id,5,2)/3) * 300 + 31
     ...
     sum(case when t21.c_ietype_code_l2 = 'XTBC' and t23.l_setup_date <= substr(t2.l_day_id,1,4) * 10000 + trunc(substr(t2.l_day_id,5,2)/3) * 300 + 31
              then t2.f_planned_eoy
              when t21.c_ietype_code in ('XTFXBC','TZZYFX') and t23.l_setup_date <= substr(t2.l_day_id,1,4) * 10000 + trunc(substr(t2.l_day_id,5,2)/3) * 300 + 31
              then t2.f_planned_eoy * -1
     ...
     sum(case when t21.c_ietype_code_l2 = 'XTCGF' and t23.l_setup_date <= substr(t2.l_day_id,1,4) * 10000 + trunc(substr(t2.l_day_id,5,2)/3) * 300 + 31
              then t2.f_planned_eoy
              when t21.c_ietype_code = 'XTFXCGF' and t23.l_setup_date <= substr(t2.l_day_id,1,4) * 10000 + trunc(substr(t2.l_day_id,5,2)/3) * 300 + 31
              then t2.f_planned_eoy * -1
     ...
     sum(case when t21.c_ietype_code_l1 = 'XTSR' and t23.l_setup_date >= substr(t2.l_day_id,1,4)||'0101' and t23.l_setup_date <= substr(t2.l_day_id,1,4) * 10000 + trunc(substr(t2.l_day_id,5,2)/3) * 300 + 31
              then t2.f_planned_eoy
              when t21.c_ietype_code in ('XTFXBC','XTFXCGF','TZZYFX') and t23.l_setup_date >= substr(t2.l_day_id,1,4)||'0101' and t23.l_setup_date <= substr(t2.l_day_id,1,4) * 10000 + trunc(substr(t2.l_day_id,5,2)/3) * 300 + 31
              then t2.f_planned_eoy * -1
...
temp_proj_rate as
 (select a.l_dayid as l_day_id, t3.l_object_id,t3.l_proj_id,sum(t3.f_divide_rate) as f_divide_rate
    from dim_pe_divide_scheme t3, dim_day a
   where t3.c_scheme_type = 'XM'
     and t3.c_object_type = 'BM'
     and t3.l_effective_date <= a.l_dayid
     and t3.l_expiration_date > a.l_dayid
    and t3.l_begdiv_date <= a.l_dayid
    and t3.l_enddiv_date >= a.l_dayid
     group by a.l_dayid, t3.l_object_id, t3.l_proj_id),
temp_stage_rate as
 (select a.l_dayid as l_day_id, t4.l_object_id,t4.l_proj_id,t4.l_divide_id as l_stage_id,sum(t4.f_divide_rate) as f_divide_rate
    from dim_pe_divide_scheme t4, dim_day a
   where t4.c_scheme_type = 'QC'
     and t4.c_object_type = 'BM'
     and t4.l_effective_date <= a.l_dayid
     and t4.l_expiration_date > a.l_dayid
      and t4.l_begdiv_date <= a.l_dayid
    and t4.l_enddiv_date >= a.l_dayid
     group by a.l_dayid, t4.l_object_id, t4.l_proj_id,t4.l_divide_id)
select a.l_day_id,
       e.l_dept_id as l_object_id,
       e.c_dept_code,
       e.c_dept_name,
       c.l_proj_id,
       c.c_proj_code,
       ...
       b.l_setup_date,
       b.l_preexp_date,
       (case when b.l_setup_date < substr(a.l_day_id,1,4)||'0101' then '0' when b.l_setup_date >= substr(a.l_day_id,1,4)||'0101' then '1' else null end) as c_status,
       (case when b.l_setup_date < substr(a.l_day_id,1,4)||'0101' then '存续' when b.l_setup_date >= substr(a.l_day_id,1,4)||'0101' then '新增' else null end) as c_status_name,
       g.f_scale_agg as f_scale_eot,
       (case when substr(b.l_setup_date,1,6) = substr(a.l_day_id,1,6)
             then (case when d.c_property_type = '1' and b.l_period <> 0 then 0
                        else g.f_scale_agg end) else 0 end) as f_netinc_scale,
       f.f_xtgdbc,
       f.f_xtfdbc,
       f.f_xtdlzf,
       f.f_xtbc,
