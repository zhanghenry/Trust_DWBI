...
select c.c_proj_code, c.c_proj_name,sum(t.f_balance_eot) as f_scale
 from tt_sr_scale_proj_m t, dim_pb_project_biz a, dim_pb_institution b, dim_pb_project_basic c
where t.l_proj_id = a.l_proj_id and a.l_bankrec_sub = b.l_inst_id 
  and a.l_proj_id = c.l_proj_id
  and t.l_month_id = to_char(To_Date('31-08-2016', 'dd-mm-yyyy'),'yyyymm') 
...
--分行收入细分
select t.c_inst_name,sum(decode(t.c_fdsrc_name,'1直投',t.f_actual_year,0)) as 直投,sum(decode(t.c_fdsrc_name,'2理财',t.f_actual_year,0)) as 理财,sum(decode(t.c_fdsrc_name,'3其他',t.f_actual_year,0)) as 其他 from (
...
with temp_fdsrc as
 (select row_number() over(partition by c.c_proj_code order by b.c_fdsrc_code,case
   ...
   where a.l_fdsrc_id = b.l_fdsrc_id(+)
     ...
     and a.l_month_id = 201610 --and c.c_proj_code = 'E701'
  ),
temp_actual_year as
 (select b.c_proj_code, sum(a.f_actual_tot) as f_actual_year
   ...
   where a.l_proj_id = b.l_proj_id
     and a.l_month_id >= substr(b.l_effective_date, 1, 6)
     and a.l_month_id < substr(b.l_expiration_date, 1, 6)
     and substr(a.l_month_id, 1, 4) = 2016
     and a.l_month_Id <= 201610 --and b.c_proj_code = 'E701'
   group by b.c_proj_code, b.c_proj_name)
...
--and s.c_proj_code = 'F466'
    ...
    select * from dim_pb_institution t where t.c_inst_name like '%交通银行%'; 
...
--部门规模类别核对
Select c.c_deptname,a.l_monthid,c.c_deptcode,Sum(a.f_new_inc_scale) As f_new_scale,Sum(a.f_clear_scale) As f_clear_scale
  From tt_project_monthly a, dim_projectinfo b, dim_department c
 Where a.l_projectid = b.l_projectid
   And b.l_deptid = c.l_deptid
   And a.l_monthid = 201605
Group By c.l_deptid,c.c_deptcode,c.c_deptname,a.l_monthid
Order By a.l_monthid,c.c_deptname;


--客户经理规模类别核对
Select  a.l_monthid, c_trustmanager,Sum(a.f_new_inc_scale) As f_new_scale,Sum(a.f_clear_scale) As f_clear_scale
  From tt_project_monthly a, dim_projectinfo b
 Where a.l_projectid = b.l_projectid
   And a.l_monthid = 201605
Group By a.l_monthid, c_trustmanager
Order By a.l_monthid, c_trustmanager;
