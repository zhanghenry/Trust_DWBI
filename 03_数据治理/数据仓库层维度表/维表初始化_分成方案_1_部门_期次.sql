--删除临时表
drop table temp_fcfacsh_bm_qc_01;
drop table temp_fcfacsh_bm_qc_02;
drop table temp_fcfacsh_bm_qc_03;
drop table temp_fcfacsh_bm_qc_04;

--员工期次分成初始化
--step1
--Dock层分成方案表与EDW层期次维度表关联
create table temp_fcfacsh_bm_qc_01 as 
...
create table temp_fcfacsh_bm_qc_02 as 
with temp_bm as
 (select t.c_dept_code,
         max(t.l_dept_id) as l_max_id,
         min(t.l_dept_id) as l_min_id,
    ...
    from dataedw.dim_pb_department t
   group by t.c_dept_code)
select a.l_dept_id,
       a.c_dept_code,
       a.c_dept_name,
       case
         when a.l_dept_id = b.l_min_id then
          20100101
         else
          a.l_effective_date
       end as l_effective_date,
       case
         when a.l_dept_id = b.l_max_id then
          20991231
         else
          a.l_expiration_date
       end as l_expiration_date,
       a.l_effective_flag
  from dataedw.dim_pb_department a, temp_bm b
 where a.c_dept_code = b.c_dept_code;

--step3
--将上一步的结果与EDW层的部门维度表关联
create table  temp_fcfacsh_bm_qc_03 as 
select t.* from (
  select '1'||lpad(row_number()over(order by a.c_scheme_code,greatest(a.l_effective_date,b.l_effective_date)),5,0) as l_scheme_id,
         a.c_scheme_code,
         a.l_proj_id,
         a.c_scheme_type,
         a.c_scheme_type_n,
    ...
    from temp_fcfacsh_bm_qc_01 a, temp_fcfacsh_bm_qc_02 b
...
create table temp_fcfacsh_bm_qc_04 as
select t.c_scheme_code,
       ...
       max(t.l_expiration_date) as l_max_expiration_date,
       min(t.l_expiration_date) as l_min_expiration_date
  from temp_fcfacsh_bm_qc_03 t
 group by t.c_scheme_code;
 
update temp_fcfacsh_bm_qc_03 t
   set t.l_effective_date = t.l_begdiv_date
 where t.l_scheme_id in (select t1.l_min_id from temp_fcfacsh_bm_qc_04 t1);
 
commit;

select *  from temp_fcfacsh_bm_qc_03 t where t.c_object_name = '结构金融部' and t.l_divide_id in (1028,2150);
