--删除临时表
drop table temp_fcfacsh_bm_xm_01;
drop table temp_fcfacsh_bm_xm_02;
drop table temp_fcfacsh_bm_xm_03;
drop table temp_fcfacsh_bm_xm_04;

--员工期次分成初始化
--step1
--Dock层分成方案表与EDW层期次维度表关联
create table temp_fcfacsh_bm_xm_01 as 
...
create table temp_fcfacsh_bm_xm_02 as 
with temp_bm as
...
create table  temp_fcfacsh_bm_xm_03 as 
select t.* from (
  select '2'||lpad(row_number()over(order by a.c_scheme_code,greatest(a.l_effective_date,b.l_effective_date)),5,0) as l_scheme_id,
         a.c_scheme_code,
         a.l_proj_id,
         a.c_scheme_type,
         a.c_scheme_type_n,
    ...
    from temp_fcfacsh_bm_xm_01 a, temp_fcfacsh_bm_xm_02 b
...
create table temp_fcfacsh_bm_xm_04 as
select t.c_scheme_code,
       ...
       max(t.l_expiration_date) as l_max_expiration_date,
       min(t.l_expiration_date) as l_min_expiration_date
  from temp_fcfacsh_bm_xm_03 t
 group by t.c_scheme_code;
 
update temp_fcfacsh_bm_xm_03 t
   set t.l_effective_date = t.l_begdiv_date
 where t.l_scheme_id in (select t1.l_min_id from temp_fcfacsh_bm_xm_04 t1);
 
commit;

select * from temp_fcfacsh_bm_xm_03;
